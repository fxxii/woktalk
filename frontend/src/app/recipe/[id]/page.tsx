    ...
    const fetchRecipe = async () => {
      ...
      try {
        ...
        if (videoId === 'GNGGWBWkRbE') {
          const response = await fetch('/demo_recipe.json');
          if (!response.ok) throw new Error('Failed to load local demo recipe');
          data = await response.json();
          // Wrap in data property to match API structure if needed, 
          // but here we can just set it directly if we want.
          // Let's assume the local JSON matches the 'data' field of the API response
          // or we adjust the logic below.
